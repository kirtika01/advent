...
exports.acceptWFH = async (req, res) => {
    try {
        ...
        attendance.acceptanceComment = req.body.acceptanceComment
        attendance.attendanceStatus = "WFH Accepted"

        let doc =await attendance.save()

        if(!doc){
            throw new Error("Unable to accept WFH attendance")
        }
        return res.status(200).json({
            status:true,
            message:"WFH attendance accepted",
